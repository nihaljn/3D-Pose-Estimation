...
class MultiViewDataset(Dataset):
    ...
    def __init__(self, keypoints_2d, pose_3d, cameras, keypoints_metadata, skeleton, fps, mode='frame'):
        ...
        if mode == 'sequence':
            self.poses_3d = [np.asarray(p_3d, dtype=np.float32) for p_3d in pose_3d]
            self.poses_2d = [np.asarray(p_2d, dtype=np.float32) for p_2d in keypoints_2d]
            self.cameras = [np.asarray(c, dtype=np.float32) for c in cameras]
        self.mode = mode
