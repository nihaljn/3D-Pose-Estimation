...
def validate(epoch, dataloader, criterion, device, model, visualize_frame=False, dataset=None, output_fp=None):
    ...
    if visualize_frame:
        ...
        data_2d = batch[0][0].unsqueeze(0).numpy()
        pred_3d = preds[0].unsqueeze(0).cpu().numpy()
        targ_3d = batch[1][0].unsqueeze(0).numpy()
...
def run(n_epochs, train_loader, val_loader, criterion, device, model, optimizer, wandb=False, 
