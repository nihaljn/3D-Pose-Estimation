...
def visualize(points_2d, gt_points_3d, pred_points_3d, points_2d_metadata, cam, skeleton, fps, output_fp):
    pred_points_3d[:, 1:] += pred_points_3d[:, :1]
    gt_points_3d[:, 1:] += gt_points_3d[:, :1]
    
    pred_points_3d = camera_to_world(pred_points_3d, R=cam['orientation'], t=cam['translation'])
    ...
    render_animation(points_2d, points_2d_metadata, anim_output,
                     ...
                     limit=20, viewport=(cam['res_w'], cam['res_h']))
    return
...
def render_animation(keypoints, keypoints_metadata, poses, skeleton, fps, bitrate, azim, output, viewport,
    ...
    keypoints_metadata: metadata about keypoints
    poses: dictionary with prediction and ground_truth
    skeleton: skeleton obj
    """
    plt.ioff()
